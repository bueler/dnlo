...
PetscErrorCode IceCtxGetL(double *L, IceCtx *user) {
    PetscFunctionBeginUser;
    *L = user->domeL;
    PetscFunctionReturn(0);
}

PetscErrorCode IceCtxGetC(double *C, IceCtx *user) {
    PetscFunctionBeginUser;
    *C = user->Gamma;
    PetscFunctionReturn(0);
}

PetscErrorCode IceCtxGetq(double *q, IceCtx *user) {
    PetscFunctionBeginUser;
    *q = user->n + 2.0;
    PetscFunctionReturn(0);
}

PetscErrorCode IceCtxGetp(double *p, IceCtx *user) {
    PetscFunctionBeginUser;
    *p = user->n + 1.0;
    PetscFunctionReturn(0);
}
...
// b(x,y) is sum of a few sines coming from vaguely-random frequencies and
// coeffs generated by fiddling; see randbed.py
double rollingbed(double x, double y) {
    const int    nc = 4,
                 jc[4] = {1, 3, 6, 8},
                 kc[4] = {1, 3, 4, 7};
